...
from modules.whisper.utils import exact_div, format_timestamp, optional_int, optional_float, str2bool, write_txt, write_vtt, write_srt

if TYPE_CHECKING:
    from modules.whisper.model import Whisper
        ...
        *, start: float, end: float, text_tokens: np.ndarray, result: DecodingResult
        ...
        if verbose:
            ...
            if signal_return is not None:
                signal_return.emit(f"[{format_timestamp(start)} --> {format_timestamp(end)}] {text}")
